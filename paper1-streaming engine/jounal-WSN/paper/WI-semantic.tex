...
The streaming semantic query provides us with a basic function to discover and  query the WOT resources in real time. We implement the following 6 query examples to illustrate its applications (Table I).  All streaming queries are showed in the Appendix A.
...
Firstly, we can get the correlation among relevant variables: TD$_{SA}$, Q, N$_s$. From Figure 8 and 9 in the Appendix C we can see that TD$_{SA}$ and Q increase with the increasing numbers of sensors. But when the ratio of TD$_{SA}$ to D reaches a certain value, the throughput will decrease rapidly. This is because the computing capability of the system will not be able to catch up with the rate of the input data stream. As a result, the delay caused by the last time slice will have an effect on the next process and the system will become more and more unstable. The evident increasing of $\sigma$ also reflects the result that TD$_{SA}$ has a larger fluctuation. The correlation analysis can help us control the rate of input stream and set proper time slice to accommodate the ability of the system.
