...
SparkRDF models the index files and intermediate results using an unified concept called RDSG(Resilient Discreted Semantic Graph). It is the basic abstraction of index files in the distributed memory. Based on the API of Spark, we provide the following three main operators with which we implement query processing.
...
\item \textbf{RDSG\_Filter:} The operator filters RDSG based on the subject or object constrains given in a TP i.e., RDSG\_Filter(RDSG1,"University0", flag) filters the departments whose parent organization is not "University0". Flag is a parameter that specifies whether the constrain is subject or object.

\item \textbf{RDSG\_Align:} The operator filters RDSG based on the subject or object constrains given in a TP i.e., RDSG\_Filter(RDSG1,"University0", flag) filters the departments whose parent organization is not "University0". Flag is a parameter that specifies whether the constrain is subject or object.

\item \textbf{RDSG\_Join:} Generally, a query is comprised of multiple subpatterns that are linked together to form the final graph pattern. The operator implements join operation on a set of RDSG and takes in as parameters, the labels of the two types of RDSG to be joined and a join condition e.g. RDSG\_Join(RDSG1, RDSG2, RDSG1.S=RDSG2.O) returns the result of the subpattern consisting of TP3 and TP4
\end{enumerate}
...
%这里需要画一个图
\begin{itemize}
\item TP$_1$: (?X rdf:type ub:Student)
\item TP$_2$: (?Y rdf:type ub:Department)
\item TP$_3$: (?Z rdf:type ub:Professor)
\item TP$_4$: (?X ub:membeOf ?Y)
\item TP$_5$: (?Y ub:subOrganizationOf University0)
\item TP$_6$: (?X ub:emailAddress ?W)
\item TP$_7$: (?X rdf:hasAdvisor ?Z)
\end{itemize}

\subsection{Overview}
We represent a SPARQL query by a query graph Q, which contains multiple query variables(such as X,Y,Z...). Each variable exists in multiple triple patterns. Figure shows the query graph corresponding to Example 1.

With Q defined, the problem of SPARQL query processing can be transformed to the problem of subgraph matching. The solution is conducted as follows: Firstly decompose Q into an ordered sequence of variables: X, Y, Z. Every query variable is made up of several ordered TPs. TP$_{1}^{X}$ refers to the first triple pattern of variable X. To every variable, we iteratively compute the matches for each TP$_{i}^{X}$, and the matched intermediate results are used to find the matches for TP$_{i+1}^{X}$ by implementing joins on the shared variable X. After computing the bindings of a variable, we continue computing the bindings of another variable based on the intermediate results of X in the same way.
At last, the remaining results are the final answer of the query graph. We will discuss the process in detail in next section.
\subsection{Query Plan}
\emph{Definition 5:}\textbf{ Query Plan.} A sequence of jobs that give the variable join order and triple pattern execution order of a SPARQL query. Every job contains multiple TPs corresponding to a variable.

The query process can be explained by a query plan. For convenience, we only refer to the TPs by the variable in that pattern(or its order subscript). For example, the TP (?X rdf:type ub:Student) will be represented as simply X and TP(?Y rdf:type ub:Department) should be denoted as Y$_{2}$ to distinguish TP$_{5}$. Thus the whole query can be simplified as follows:{X,Y$_{2}$,Z,XY,Y$_{5}$,XW,XZ}. Given the query, three possible query plans are as follows:
Plan 1. job1={X,XY,XW,XZ}, job2={Y$_{2}$,Y$_{5}$},job3={Z}

Plan 2. job1={Y$_{2}$,XY,Y$_{5}$}, job2={X,XW,XZ},job3={Z}

Plan 3. job1={Y$_{5}$,Y$_{2}$,XY}, job2={X,XW,XZ},job3={Z}

For single triple pattern matching(as is shown in Figure ), we use the RDSG-based operators to get the bindings of variables in the triple pattern. Firstly, RDSG_Gen is used to choose appropriate index files to create a RDSG. Then the Prepartition strategy guarantees that the data sharing the same variable value will be read into the same partition. Then RDSG_Filter further filters lots of unrelated bindings
of a given variable based on the subject or object of the TP. 

As the TPs owning X may also contain other variables, the intermediate results format is (Binding(X), Binding(Other)). The former computes the bindings of X, and the latter gets the bindings of other variables such as Y.
For multiple triple pattern matching in a job, 


\begin{figure*}[htbp]
\centering
\includegraphics[width=1\textwidth,bb=0 0 818 391]{join.pdf}
\caption{Join Processing of SparkRDF}
\label{}
\end{figure*}

\subsection{Cost Estimation}
We run TP-based Spark jobs to answer a SPARQL query. Every job mainly contains three tasks, which are reading, matching and joining.
The cost of reading and matching TP$_{i}$, which are both in positive correlation with the size of index files, are denoted as $|$Read(TP$_{i}$)$|$ and $|$Match(TP$_{i}$)$|$. The cost of joining two RDSG is composed of shuffle communication cost in distributed environment and join computation cost in single node: Shuffle(RDSG1,RDSG2), Computation(RDSG1,RDSG2), both of which are roughly proportional to the size of RDSG. The cost parameters are assigned a different weight according to the bandwidth of disk, network and memory.
%is estimated as
...
\begin{equation}
\begin{aligned}
COST= &\sum_{i=1}^n (\mu|Read(TP_i)|+\nu|Match(TP_i)|)+\\
&\sum_{i=2}^n Join(Result_{i-1},Match(TP_i))\\
=&\sum_{i=2}^n Job_i+\mu|Read(TP_1)|+\nu|Match(TP_1)|
\end{aligned}
\end{equation}
...
\begin{array}
{l@{\quad:\quad}l}
Join(Result_{i-1},Match(TP_i)) & $i$>=2 \\
Match(TP_i) & $i$=1
\end{array}
\right.
\end{equation}
...
\   \   \   \   Read(TP$_{i}$)=the result size of loading selected input index files into RDSG.

\   \   \   \   Match(TP$_{i}$)=the result size for single triple pattern matching TP$_{i}$.

\   \   \   \   Result$_{i}$=the intermediate results size for TP$_{1}$, ... ,TP$_{i}$.

\   \   \   \   Join(RDSG1,RDSG2)=the cost of joining RDSG1 and RDSG2.

\   \   \   \   Shuffle(RDSG1,RDSG2)=the size of the data needed to be moved in distributed environment.

\   \   \   \   Computation(RDSG1,RDSG2)=the size of the data needed to be implemented join operation in single node.



Equation(1) and Equation(2) estimate the total cost of processing a query. It is the summation of
the (n-1) individual job costs and the reading and matching cost of the first job(n is the number of TP in the query). We can reduce the total query cost by two ways: cut down the number of jobs and the cost of every job.
The former can be achieved by the optimization strategy called merging TP(see section ). The latter can be implemented
...
\emph{Definition 2:}\textbf{ Selectivity Score (SS)}. The evaluation parameter that is used to estimate the result size of a single triple pattern based on the type of .

\emph{Definition 3:}\textbf{ Candidate Joining Variable (CV).} A variable that exists in two or more TPs.

\emph{Definition 4:}\textbf{ Joining Variable (JV)}. The CJV that has the highest Selectivity Score.



Before describing our join optimization processing, we would like to point some definitions and facts about the join process.

\begin{theorem}
In Semantic Web, every identified entity belongs to a specific class, denoted by predicate "type". The implicit type message can be sent to its corresponding TPs to form a new and more restrictive TP in a SPARQL query, which enables to improve the query efficiency greatly.
\end{theorem}
\textit{PROOF.}

\begin{theorem}
SPARQL query can be regarded as the process of iterative joining, in which later joins depend on previous intermediate join results. The total join cost can be minimized by choosing the most selective \textbf{CV} as \textbf{ Joining Variable}.
\end{theorem}
\textit{PROOF.}
